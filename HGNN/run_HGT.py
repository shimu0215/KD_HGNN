...
def train_HGT(args, data):
    ...
    model = HGT(hidden_channels=64, out_channels=num_class, num_heads=2, num_layers=1, data=data)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    data, model = data.to(device), model.to(device)

    with torch.no_grad():  # Initialize lazy modules.
        out = model(data.x_dict, data.edge_index_dict, node_type)
    ...
    with torch.no_grad():
        predictions = model(data.x_dict, data.edge_index_dict, node_type)
